    ...
    <aside className="w-full h-full bg-white/95 backdrop-blur-sm border-r border-gray-200/50 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="p-4 lg:p-6 border-b border-gray-100 flex-shrink-0">
          <div className="flex items-center gap-3 lg:gap-4 mb-4">
            <div className="relative">
              <div className="h-12 w-12 lg:h-14 lg:w-14 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-sm lg:text-lg font-semibold">
                {user.fullName.split(' ').map(n => n[0]).join('').slice(0, 2)}
              </div>
              <div className="absolute -bottom-1 -right-1 h-3 w-3 lg:h-4 lg:w-4 bg-green-400 rounded-full border-2 border-white"></div>
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-gray-900 truncate text-sm lg:text-base">{user.fullName}</h3>
        ...
        <div className="p-4 lg:p-6 border-b border-gray-100">
          <h3 className="text-xs lg:text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2">
            ...
            {courses.map((course) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-xs lg:text-sm font-medium truncate">{course.name}</div>
        ...
        <div className="p-4 lg:p-6 flex-1">
          <h3 className="text-xs lg:text-sm font-semibold text-gray-900 mb-3">Navegación</h3>
          <nav className="space-y-1">
            {moduleItems.map(({ id, label, icon: Icon }) => (
              <button
                key={id}
                onClick={() => onModuleSelect(id as ActiveModule)}
                className={`w-full flex items-center gap-3 px-3 py-2 lg:py-2.5 rounded-lg transition-all duration-200 ${
                  activeModule === id
